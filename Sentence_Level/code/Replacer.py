...
def removeNonEnglishWords(tweet, token):
    """Removes the non-english or non-ascii characters takes as input a list of words in tweet
       and a list of corresponding tokens, not using tokens now but may use in future
...
def removeStopWords(tweet, token, stopWordsDict):
    """Removes the stop words, takes as input a list of words in tweet ,a list of corresponding tokens
       and a stopWords Dictionary,and return the modified list of token and words"""
...
def replaceEmoticons(emoticonsDict, tweet, token):
    """Replaces the emoticons present in tweet with its polarity takes as input a emoticons dictionary
       which has emoticons as key and polarity as value and a list which contains words in tweet and
...
def expandAcronym(acronymDict, tweet, token):
    """Expand the Acronym present in tweet takes as input a acronym dict which has acronym
       as key and abbreviation as value, a list which contains words in tweet and a list of
       token and return list of words in tweet after expansion and tokens"""
    newTweet = []
    newToken = []
    count = 0
    for i in range(len(tweet)):
...
def replaceRepetition(tweet):
    """Takes as input a list which contains words in tweet and return list of words in tweet
        after replacement and numner of repetion
...
def replaceNegation(tweet):
    """Takes as input a list which contains words in tweet and return list of words in tweet
       after replacement of "not","no","n't","~"
       eg isn't -> negation
          not -> negation """
    for i in range(len(tweet)):
        word = tweet[i].lower().strip(specialChar)
