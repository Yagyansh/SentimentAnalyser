...
def removeNonEnglishWords(tweet, token):
    """Removes the non-english or non-ascii characters.
...
def removeStopWords(tweet, token, stopWordsDict):
    """Removes the stop words.
       Takes as input a list of words in tweet ,a list of corresponding tokens
       and a stopWords Dictionary.
       Returns the modified list of token and words"""
...
def replaceEmoticons(emoticonsDict, tweet, token):
    """Replaces the emoticons present in tweet with its polarity.
       Takes as input a emoticons dictionary which has emoticons as key and polarity as value
...
def expandAcronym(acronymDict, tweet, token):
    """Expand the Acronym present in tweet.
       Takes as input a acronym dict which has acronyms key and abbreviation as value,
       a list which contains words in tweet and a list of token.
       Returns list of words in tweet after expansion and tokens"""
...
def replaceRepetition(tweet):
    """Takes as input a list which contains words in tweet.
       Returns list of words in tweet after replacement and number of repetitions.
...
def replaceNegation(tweet):
    """Takes as input a list which contains words in tweet.
       Returns list of words in tweet after replacement of "not","no","n't","~"
       eg isn't -> negation 
          not -> negation """
    for i in range(len(tweet)):
        word = tweet[i].lower().strip(specialChar)
